...
<div v-if="categoriesModal" class="fixed z-10 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
   <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
    ...
    <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
          ...
          <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
            ...
            <div class="mt-2">
              <p class="text-sm text-gray-700">
                Está quase tudo pronto para começar a usar a plataforma Cotamaq.  </p>
                <p class="text-sm text-gray-700"> Para continuar, escolha uma ou mais categorias de venda.
              </p>
            </div>
            <div class="mt-2 flex justify-content items-center">
              <div class="md:w-custom mx-auto py-8 md:flex md:justify-start md:flex-wrap" >
               <div class="w-32 h-32 mr-4 bg-white rounded-lg shadow-md p-6 cursor-pointer mb-8 hover:bg-green-lightest focus:outline-none focus:shadow-outline-green"  tabindex="0" @click="toggleCategories(category.id)" v-for="category in categories" :key="category.id">
      <div class="flex grid justify-center items-center mb-3">
        <div class="flex justify-center items-center" >
          <div :class="selectedCategories.includes(category.id) ? 'bg-primary-lighter':'bg-gray-400'" class="mx-auto flex-shrink-0 flex items-center justify-center h-8 w-8 rounded-full sm:mx-0 sm:h-8 sm:w-8">
            <i class="mdi mdi-checkbox-multiple-marked text-white text-md"></i>
          </div>
        </div>
        <h2 class="text-base text-blue-darker my-2">{{category.name}}</h2>
      ...
      <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
        <button @click="attachCategories()" type="button" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-primary-main text-base font-medium text-white hover:bg-primary-lighter focus:outline-none focus:ring-2 focus:ring-offset-2 sm:ml-3 sm:w-auto sm:text-sm">
          Salvar
        </button>
      </div>
    </div> 
    ...
    >
      <!-- Desktop sidebar -->
      <aside     :style="ModalOpen  ? 'pointer-events:none; opacity: 0.6':''" 
      ...
      >
        <div  class="py-4 text-gray-500  dark:text-gray-400">
          <a
            ...
            href="#"
          >
            <img class="mx-auto h-12 w-auto "  src="../assets/images/logo-01.png" alt="">
          </a>
          <ul class="mt-6">
            ...
            <li class="relative px-6 py-3 hover:text-gray-800 " style="cursor: pointer">
              ...
              ></span>
              <router-link 
               v-on:click.native="closePagesMenu" :to="{path: '/cotacoes'}"  class="inline-flex items-center w-full text-sm font-semibold text-gray-700 transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200"
              >
            ...
            <li class="relative px-6 py-3 hover:text-gray-800 " style="cursor: pointer">
              ...
              ></span>
              <router-link 
               v-on:click.native="closePagesMenu" :to="{path: '/pedidos'}" class="inline-flex items-center w-full text-sm font-semibold text-gray-700 transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200"
              >
          ...
          <ul>
            <li class="relative px-6 py-3">
              <span
                v-if="getRouteName == 'payments' || getRouteName == 'companies' || getRouteName == 'addresses'"
                class="absolute inset-y-0 left-0 w-1 bg-primary-main rounded-tr-lg rounded-br-lg"
                aria-hidden="true"
              ></span>
              <button
              ...
              <template v-if="isPagesMenuOpen">
                ...
                >
                  ...
                  >
                  <router-link 
                   v-on:click.native="closePagesMenu" :to="{path: '/enderecos'}" class="w-full text-gray-600"
                  >
                  Endereços
                  </router-link >
                  </li>
                  ...
                  >
                    <router-link 
                      ...
                      :to="{path: '/pagamentos'}" class="w-full text-gray-600"
                    >
                        Pagamentos
                    </router-link>
                  </li>
                  ...
                  >
                  <router-link 
                   v-on:click.native="closePagesMenu" :to="{path: '/empresa'}" class="w-full text-gray-600"
                  >
                    Empresa
                  </router-link>                 
                  </li>
                  ...
                  >
                  <router-link 
                    :to="{path: '/equipamentos'}" class="w-full text-gray-600"
                  >
                    Equipamentos
                  </router-link>
                  </li>
            ...
            <li class="relative px-6 py-3">
              ...
              <template v-if="isPagesMenuOpen">
                ...
                >
                  ...
                  >
                  <router-link 
                   v-on:click.native="closePagesMenu" :to="{path: '/enderecos'}" class="w-full "
                  >
                  Endereços
                  </router-link >
                  </li>
                  ...
                  >
                    <router-link 
                      :to="{path: '/pagamentos'}" class="w-full"
                    >
                        Pagamentos
                    </router-link>
                  </li>
      ...
      <div class="flex flex-col flex-1">
        <header :style="ModalOpen  ? 'pointer-events:none; opacity: 0.6':''"  class="z-10 py-4 bg-primary-main shadow-md dark:bg-gray-800">
          <div
                ...
                <template v-if="isProfileMenuOpen">
                  ...
                  <li class="flex">
                      <a class="inline-flex items-center w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md hover:bg-gray-100 hover:text-gray-800 dark:hover:bg-gray-800 dark:hover:text-gray-200">
                        <span class="w-full flex justify-center items-center text-white bg-primary-lighter px-4 py-1 text-sm rounded mx-2 font-semibold"> {{ user.company }}</span>
                      </a>
                  </li>
                    <li class="flex">
...
export default {
    ...
    created() {
      ...
      bus.$on('updatedUser', (data) => {
        if(data) {
          ...
          if(this.user.first_login == 0 && this.user.role == 1 && this.user.has_categories == 0)
          {
    ...
    attachCategories() {
      if(this.selectedCategories.length === 0) {
        ...
        userService.toggleCategories(payload).then((response) => {
