        ...
        <div  @click="setCurrentConversation(conversation)" :class="currentConversationId == conversation.id ? 'bg-green-100 hover:none' : 'hover:bg-gray-200'"  class="flex items-center cursor-pointer rounded-md py-2 px-2">
          <div class="flex items-center w-full">
            ...
            <div class="ml-3 w-full">
              ...
              <span :class="conversation.lastMessageIsImage == 1 && 'flex items-center justify-between'">
                <svg v-if="conversation.lastMessageIsImage == 1" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-500 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                <div class="flex items-center justify-between">
                  <p v-if="conversation.lastMessageIsImage == 1" class="text-gray-500 text-sm truncate w-64">Imagem</p>
                  <p v-else class="text-gray-500 text-sm truncate w-64">{{conversation.userSend == userId ? `Você: ${conversation.lastMessage}` : `${conversation.lastMessage}`}}</p>
