<template>
  <div class="w-2/6 pt-3 h-screen">
    ...
    <div v-else @click="loadConversation()" v-for="conversation in filteredConversations" :key="conversation.id" class="overflow-hidden hover:bg-gray-200 flex items-center cursor-pointer rounded-md py-2 px-2">
      <div class="flex items-center w-full">
        ...
        <div class="ml-3 w-full">
          <div class="w-full items-center flex justify-between">
            <h5 class="font-bold truncate w-72">{{ conversation.user }}</h5>
            <p class="text-gray-500 text-sm">{{conversation.lastMessageDate}}</p>
          </div>
          <p class="text-gray-500 text-sm truncate w-72">{{ conversation.lastMessage }}</p>
        </div>
      </div>
