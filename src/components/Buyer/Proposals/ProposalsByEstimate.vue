    ...
    <div class="flex flex-col">
      ...
      >
        ...
        >
          ...
          <button
            v-tooltip="{ content: 'Click para aplicar desconto à vista' }"
            v-else
            @click="applyDiscont"
            :class="formatItem(3).bg"
            class="
...
export default {
  ...
  computed: {
    sortPrice() {
      return {
        "mdi-sort": this.sorting.price.default,
        "mdi-sort-ascending": this.sorting.price.ascending,
        "mdi-sort-descending": this.sorting.price.descending,
      };
    },
  ...
  methods: {
    ...
    applyDiscont() {
      ...
      this.proposals.forEach((proposal) => {
        ...
        if (proposal.discount < 10) {
          result =
            proposal.subtotal -
            proposal.subtotal * parseFloat("0.0" + proposal.discount);
          proposal.subtotal = result;
          proposal.total = proposal.subtotal + proposal.shipping;
        } else {
          result =
            proposal.subtotal -
            proposal.subtotal * parseFloat("0." + proposal.discount);
          proposal.subtotal = result;
          proposal.total = proposal.subtotal + proposal.shipping;
        }
      });
    ...
    setAccepted() {
      console.log("Cotação: ", this.estimate);
    },
    showEstimateModal() {
