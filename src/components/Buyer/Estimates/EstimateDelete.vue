      ...
      >
        ...
        >
            ...
            <div class="sm:flex sm:items-start">
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                <h3
                  ...
                  id="modal-headline"
                >
                  {{selectedEstimate.status == 5 ? 'Deletar cotação?':'Cancelar cotação?'}}
                </h3>
                <div class="mt-2">
                  <p class="text-sm text-gray-500">
                  {{selectedEstimate.status == 5 ? 'Tem certeza que deseja excluir essa cotação?':'Tem certeza que deseja cancelar essa cotação?'}}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            ...
            >
              {{selectedEstimate.status == 5 ? 'Apagar':'Sim'}}
            </button>
            <button
              type="button"
              @click="close"
            ...
            >
              {{selectedEstimate.status == 5 ? 'Cancelar':'Não'}}
            </button>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    deleteEstimate() {
            this.disabled = true
            const estimateId = this.selectedEstimate.id
            estimateService.deleteEstimate(estimateId).then((response) => {
