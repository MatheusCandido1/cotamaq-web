    ...
    <div>
        <div class="my-6 px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
            <div class="flex -mx-3">
                ...
                <div class="w-1/5 px-3 mb-5">
                    ...
                    <div class="flex">
                        <div class="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center"></div>
                        <router-link
                            :to="{ path: '/criar/cotacao' }"
                            class="w-full -ml-10 pl-2 pr-3 py-2 flex items-center justify-center bg-primary-main text-white font-semibold rounded border-b-2 border-primary-main hover:text-white shadow-md py-2 px-6 inline-flex items-center"
                        >
            ...
            <div class="flex -mx-3">
                ...
                <div class="w-1/5 px-3 mb-5">
                    ...
                    <div class="flex">
                        ...
                        <button
                            @click="filterData()"
                            class="w-full flex items-center justify-center bg-white text-gray-600 font-semibold rounded border-b-2 border-blue-500 hover:border-blue-600 hover:bg-blue-500 hover:text-white shadow-md py-2 px-6 inline-flex items-center"
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path
                ...
                <div class="w-1/5 px-3 mb-5">
                    ...
                    <div class="flex">
                        ...
                        <button
                            @click="clearFilter()"
                            class="w-full flex items-center justify-center bg-white text-gray-600 font-semibold rounded border-b-2 border-red-500 hover:border-red-600 hover:bg-red-500 hover:text-white shadow-md py-2 px-6 inline-flex items-center"
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div v-if="estimates && estimates.length !== 0">
                <v-table class="w-full whitespace-no-wrap mb-2" :data="filteredEstimates" :hideSortIcons="true" :currentPage.sync="currentPage" :pageSize="itemsPerpage" :filters="filter.input" @totalPagesChanged="totalPages = $event">
                    <thead class="rounded border-b-2 border-primary-main shadow-md py-2 px-6 outline-none no-selection" slot="head">
                        <tr class="text-xs h-10 font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
                            ...
                            <v-th class="text-center" style="width: 15%;" sortKey="observation">Observação</v-th>
                            <v-th class="text-center" style="width: 25%;" sortKey="status">Situação</v-th>
                    ...
                    <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800" slot="body" slot-scope="{ displayData }">
                        <tr v-for="row in displayData" :key="row.id">
                            ...
                            <td v-if="row.status == 6" class="text-sm text-center text-gray-700">
                                Não disponível
                            </td>

                            <td v-else class="text-sm text-center text-gray-700">
                                {{ row.proposals.reduce((acc, e) => { e.status == 3 ? acc++ : false; return acc; }, 0) }}
                            </td>
                            <td class="text-sm text-center text-gray-700">
                                {{ row.observation }}
                            </td>
                            <td
                                v-if="row.proposals.reduce((acc, e) => {e.status == 3 ? acc++ : false;return acc;}, 0)"
                            ...
                            <td class="flex justify-center mt-1 mb-1">
                                <div class="flex items-center space-x-2 text-sm">
                                    <router-link
                                        v-tooltip="{ content: 'Editar Cotação' }"
                                        v-if="row.status == 1"
                                        :to="{ name: 'editEstimate', params: { id: row.id } }"
                    ...
                    "
                                    ...
                                    <router-link
                                        v-tooltip="{ content: 'Pedidos' }"
                                        v-if="row.status == 6"
                                        :to="{
                      name: 'OrderDetails',
                      params: { id: row.proposals[0].order.id },
                    }"
                                        class="flex items-center justify-between px-2 py-2 bg-indigo-600 text-sm font-medium leading-5 text-white rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                        aria-label="Edit"
                                    >
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path
                                    ...
                                    <button
                                        v-tooltip="{ content: 'Cancelar/Deletar Cotação' }"
                                        v-if="row.status == 1 || row.status == 2 || row.status == 5"
                                        @click="showDeleteModal(row)"
                                        class="flex items-center justify-between px-2 py-2 bg-red-500 text-sm font-medium leading-5 text-white rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                        aria-label="Edit"
                                    >
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    <button
                                        v-tooltip="{ content: 'Duplicar Cotação' }"
                                        v-if="
                      row.status == 1 ||
                      row.status == 2 ||
                      row.status == 3 ||
                      row.status == 5 ||
                      row.status == 6 ||
                      row.proposals.reduce((acc, e) => {
                        e.status == 3 ? acc++ : false;
                        return acc;
                      }, 0)
                    "
                                        @click="showDuplicateModal(row)"
    ...
    filterData() {
      ...
      if (
        ...
        if (this.filter.situation != 0) {
          this.filteredEstimates = this.filteredEstimates.filter(
            (estimate) => estimate.status == this.filter.situation
          );
        ...
        if (this.filter.start_date != null && this.filter.end_date == null) {
          this.filteredEstimates = this.filteredEstimates.filter(
            (estimate) => estimate.created_at >= this.filter.start_date
          );
          return this.filteredEstimates;
        } else {
          return this.filteredEstimates;
        ...
        if (
          ...
          this.filteredEstimates = this.filteredEstimates.filter(
            (estimate) => estimate.created_at <= this.filter.end_date
          );
          return this.filteredEstimates;
        } else {
          return this.filteredEstimates;
        ...
        if (
          ...
          this.filteredEstimates = this.filteredEstimates.filter((estimate) =>
            estimate.created_at >= this.filter.start_date &&
            estimate.proposals_by_seller.length >= 1
              ? estimate.proposals_by_seller[0].status == this.filter.situation
              : estimate.status == this.filter.situation
          );
          return this.filteredEstimates;
        } else {
          return this.filteredEstimates;
        ...
        if (
          ...
          this.filteredEstimates = this.filteredEstimates.filter((estimate) =>
            estimate.created_at <= this.filter.end_date &&
            estimate.proposals_by_seller.length >= 1
              ? estimate.proposals_by_seller[0].status == this.filter.situation
              : estimate.status == this.filter.situation
          );
