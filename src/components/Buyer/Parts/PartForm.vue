<template>
  <div class="flex flex-col">
      ...
      <form @submit.prevent="showConfirmModal">
          ...
          <div class="md:w-1/2 px-3 mb-2 md:mb-0">
            ...
            <div class="flex">
              <select v-model="part.address_id" @change="() => (errors.part.address_id = 'OK')"
                      ...
                      class="w-full pl-2 pr-3 py-2 rounded border-b-2 shadow-md py-2 px-6 outline-none  focus:border-primary-lighter">
                <option disabled value=""> Selecione...</option>
                <option v-for="address in addresses" :key="address.id" :value="address.id">{{ address.description }}
                </option>
              </select>
              <button type="button" @click="showAddAddressModal"
                      class="bg-primary-main font-semibold text-white border-gray-400 w-10 flex rounded-r focus:outline-none cursor-pointer">
        ...
        <div class="-mx-3 md:flex mb-6 mt-2">
          <div class="md:w-full px-3 flex justify-center gap-4">
            <button @click="showEquipmentSelect" type="button"
                    class="sm:w-full md:w-1/4 flex items-center justify-center bg-gray-600 text-white font-semibold rounded hover:bg-gray-700 shadow-md py-2 px-6 inline-flex items-center">
              <span class="justify-center items-center"><i class="mdi mdi-tractor-variant mr-2"></i>Usar Equipamento Existente</span>
            </button>
            <button @click="showEquipmentForm" type="button"
                    class="sm:w-full md:w-1/4 flex items-center justify-center bg-primary-main text-white font-semibold rounded hover:bg-primary-darker shadow-md py-2 px-6 inline-flex items-center">
        ...
        <div v-if="equipmentForm == 1">
            ...
            <div class="md:w-1/3 px-3 mb-2 md:mb-0">
              ...
              <div class="flex">
                ...
                <the-mask @change.native="() => (errors.equipment.year = 'OK')"
                          :class="errors.equipment.year == 'ERROR' ? 'border-red-400':'border-primary-main'"
                          v-model="equipment.year" mask="####"
                          class="w-full -ml-10 pl-2 pr-3 py-2 rounded border-b-2 shadow-md py-2 px-6 outline-none  focus:border-primary-lighter"></the-mask>
              </div>
          ...
          <div class="-mx-3 md:flex mt-2">
            <div class="md:w-full px-3 flex justify-end gap-2">
              <button @click="handleEquipmentCancel"
                      class="sm:w-full md:w-1/3 w-full flex items-center justify-center bg-red-600 text-white font-semibold rounded hover:bg-red-700 hover:text-white shadow-md py-2 px-6 inline-flex items-center">
    ...
    <PartConfirm v-if="modal.confirm" :categories="categories" :part="part" :equipment="equipmentInfo"
                 @save="createPart" @close="closeConfirmModal"/>
  </div>
</template>
