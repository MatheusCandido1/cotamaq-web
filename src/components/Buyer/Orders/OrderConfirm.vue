<template>
  ...
  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />
              ...
              <div v-if="selectedProposal.payment_condition_id != 0 && selectedProposal.payment_method_id != 0" class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                <h3
                ...
                <div class="mt-2">
                  <p class="text-sm text-gray-500">
                   Tem certeza que deseja transformar essa proposta em pedido?
              ...
              <div v-else class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                ...
                <div class="mt-2">
                  <p class="text-sm text-gray-500">
                   Para concluir essa compra, você precisa escolher o método de pagamento e a condição de pagamento!
                  </p>
                </div>
              </div>
            </div>
          ...
          <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            <button
            v-if="selectedProposal.payment_condition_id != 0 && selectedProposal.payment_method_id != 0"
              type="button"
              :disabled="disabled"
              @click="generateOrder()"
            ...
            <button
              type="button"
              @click="close"
              class="mt-3 w-full inline-flex justify-center rounded-md border  shadow-sm px-4 py-2 bg-gray-500 text-white text-base font-medium hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-main sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"
            >
              Cancelar
            </button>
          </div>
    ...
    generateOrder() {
      this.disabled = true
        ...
        orderService.createOrder(data).then((response) => {
            ...
            this.$toast.info(error.response.data.error_message, {
                position: "bottom-right",
                pauseOnHover: false,
                showCloseButtonOnHover: true,
                timeout: 2500
