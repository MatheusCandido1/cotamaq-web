              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                ...
                <div class="mt-2">
                  <p class="text-sm text-gray-600">
                   Tem certeza que deseja alterar a situação do pedido para <span class="font-bold">{{formatStatus(newStatus).text}}</span> ?
                  </p>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    formatStatus(value) {
        let format = this.textStatus.find(status => status.id == value)
        return format
    },
  },
};
