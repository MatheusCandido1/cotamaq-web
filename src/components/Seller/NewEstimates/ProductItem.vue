    ...
    <div class="relative bg-white   overflow-hidden rounded-xl w-full my-4 shadow-xl">
        ...
        <div>
            ...
            <div>
               ...
               <div  v-if="product.proposals_by_seller && product.proposals_by_seller.length > 0"  class="flex justify-between px-2 ">
                    <div v-if="product.proposals_by_seller && product.proposals_by_seller.length == 0" class="my-2">
                        <div :class="formatEstimateStatus(product.proposals_by_seller && product.proposals_by_seller.length >= 1 ? product.proposals_by_seller[0].status:product.status).bg" class="w-full text-sm px-2 py-1 font-semibold text-white rounded-md dark:text-white">
                            {{formatEstimateStatus(product.proposals_by_seller && product.proposals_by_seller.length >= 1 ? product.proposals_by_seller[0].status:product.status).text}}
                        </div>
                    </div>
                    <div v-else class="my-2">
                        <div :class="formatProposalStatus(product.proposals_by_seller && product.proposals_by_seller.length >= 1 ? product.proposals_by_seller[0].status:1).bg" class="w-full text-sm px-2 py-1 font-semibold text-white rounded-md dark:text-white">
                            {{formatProposalStatus(product.proposals_by_seller && product.proposals_by_seller.length >= 1 ? product.proposals_by_seller[0].status:1).text}}
                        </div>
                    </div>
                            ...
                            <button
                              v-if="product.proposals_by_seller 
                                    ...
                                    && ( product.proposals_by_seller[0].status == 1 
                                        || product.proposals_by_seller[0].status == 2 
                                        || product.proposals_by_seller[0].status == 3
                                    )"
                            ...
                            <button 
                                v-if="product.proposals_by_seller 
                                    ...
                                    && ( product.proposals_by_seller[0].status == 1 
                                        || product.proposals_by_seller[0].status == 2 
                                        || product.proposals_by_seller[0].status == 3
                                    )" @click="handleNewProposalClick"   class="bg-primary-main w-full text-sm px-2 py-1 font-semibold text-white rounded-md dark:text-white">
                                Nova Proposta
                            </button>
                        </div>
                    </div>
